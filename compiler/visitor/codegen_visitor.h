...
class HexCodeGenVisitor : public AstVisitor {
public:
  HexCodeGenVisitor();
  ~HexCodeGenVisitor();
...
protected:
  void add_inst(c_str, c_str, c_str, c_str);

  template<class T, class Function>
  void iterate(std::list<T>*, Function, int startAt=0);

  template<class T, class Function>
  void iterate_with_index(std::list<T>*, Function);

  void cgen_unary_expr(HexAstUnaryExpr, c_str);
  void cgen_binary_expr(HexAstBinaryExpr, c_str);
  void cgen_pseudo_binary_expr(HexAstBinaryExpr, c_str);
};
